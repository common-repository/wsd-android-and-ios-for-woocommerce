...
class WSDAI_JSON_API_Comment {
  ...
  function WSDAI_JSON_API_Comment($wp_comment = null) {
    if ($wp_comment) {
  ...
  function import_wp_object($wp_comment) {
    ...
    if (!empty($wp_comment->user_id)) {
      $this->author = new WSDAI_JSON_API_Author($wp_comment->user_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function comment_post_redirect() {
    ...
    $new_comment = new WSDAI_JSON_API_Comment($comment);
