...
class WSDAI_JSON_API_Post {
  
  // Note:
  //   WSDAI_JSON_API_Post objects must be instantiated within The Loop.
  ...
  function WSDAI_JSON_API_Post($wp_post = null) {
    if (!empty($wp_post)) {
  ...
  function save($values = null) {
    ...
    if (!empty($_FILES['attachment'])) {
      ...
      $this->attachments[] = new WSDAI_JSON_API_Attachment($attachment_id);
    ...
    if ($json_api->include_value('categories')) {
      ...
      if ($wp_categories = get_the_category($this->id)) {
        foreach ($wp_categories as $wp_category) {
          $category = new WSDAI_JSON_API_Category($wp_category);
    ...
    if ($json_api->include_value('tags')) {
      ...
      if ($wp_tags = get_the_tags($this->id)) {
        foreach ($wp_tags as $wp_tag) {
          $this->tags[] = new WSDAI_JSON_API_Tag($wp_tag);
        }
      }
    } else {
      unset($this->tags);
  ...
  function set_author_value($author_id) {
    ...
    if ($json_api->include_value('author')) {
      $this->author = new WSDAI_JSON_API_Author($author_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function set_custom_taxonomies($type) {
    ...
    foreach ($taxonomies as $taxonomy_id => $taxonomy) {
      ...
      $taxonomy_class = $taxonomy->hierarchical ? 'WSDAI_JSON_API_Category' : 'WSDAI_JSON_API_Tag';
