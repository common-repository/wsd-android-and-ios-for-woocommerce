...
class WSDAI_JSON_API_Query {
  ...
  function __construct() {
    // Register Android and iOS App plugin for WooCommerce by WSD query vars
  ...
  function get_legacy_controller($json) {
    ...
    if ($json == 'submit_comment') {
      if ($json_api->controller_is_active('respond')) {
        ...
        $json_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > Android and iOS App plugin for WooCommerce by WSD.");
      }
    } else if ($json == 'create_post') {
      if ($json_api->controller_is_active('posts')) {
        return 'posts';
      } else {
        $json_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > Android and iOS App plugin for WooCommerce by WSD.");
      }
    } else {
      return 'core';
    }
  ...
  function get_method($controller) {
    ...
    if (empty($method)) {
      ...
      return false;
    } else if (method_exists("WSDAI_JSON_API_{$controller}_Controller", $method)) {
