...
class WSDAI_JSON_API {
  
  function __construct() {
    $this->query = new WSDAI_JSON_API_Query();
    $this->introspector = new WSDAI_JSON_API_Introspector();
    $this->response = new WSDAI_JSON_API_Response();
  ...
  function template_redirect() {
	...
	//$active_controllers = Array('core','auth','user');
    //print_r($controller);
    //print_r($available_controllers);
    //print_r($enabled_controllers);
    if ($controller) {
      
      if (empty($this->query->dev)) {
        error_reporting(0);
      }
      //print_r($active_controllers);
	  //print_r($enabled_controllers);
	  //echo $controller;
      ...
      if ($method) {
        ...
        do_action("WSDAI_json_api", $controller, $method);
        do_action("WSDAI_json_api-{$controller}-$method");
  ...
  function admin_menu() {
    add_options_page('WSD Settings', 'WSD Settings', 'manage_options', 'json-api', array(&$this, 'admin_options'));
  }
...
<div class="wrap">
  ...
  <h2>Android and iOS App plugin for WooCommerce by WSD Settings</h2>
  <form action="options-general.php?page=json-api" method="post">
    ...
    <h3>Address</h3>
    <p>Specify a base URL for Android and iOS App plugin for WooCommerce by WSD. For example, using <code>api</code> as your API base URL would enable the following <code><?php bloginfo('url'); ?>/api/get_recent_posts/</code>. If you assign a blank value the API will only be available by setting a <code>json</code> query variable.</p>
...
<?php
  ...
  function check_directory_for_controllers($dir, &$controllers) {
    ...
    while ($file = readdir($dh)) {
      if (preg_match('/(.+)\.php$/i', $file, $matches)) {
        ...
        if (preg_match("/class\s+WSDAI_JSON_API_{$matches[1]}_Controller/i", $src)) {
  ...
  function controller_class($controller) {
    return "WSDAI_json_api_{$controller}_controller";
  }
  
  function controller_path($controller) {
	  
    $WSDAI_json_api_dir = WSDAI_json_api_dir();
    $json_api_path = "$WSDAI_json_api_dir/controllers/$controller.php";
